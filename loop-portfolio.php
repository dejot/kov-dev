...
<?php
	while ( have_posts() ) : the_post(); $count++;
		...
		if ( isset( $woo_options['woo_portfolio_linkto'] ) && ( $woo_options['woo_portfolio_linkto'] == 'post' ) ) {
			...
			$settings['large'] = '#';
			$gallery_id = 'data-gallery-id="' . get_the_ID() . '"';
		}
...
?>
		<article id="portfolio-item-id-<?php the_ID(); ?>" <?php post_class( 'portfolio-item' ); ?>>
			...
			?>
			<a <?php echo $gallery_id; ?> title="<?php echo $settings['caption']; ?>" href="<?php echo $settings['large']; ?>" class="item drop-shadow curved curved-hz-1">
				<?php echo $image; ?>
				<span class="mask">
			...
			<h2><a <?php echo $gallery_id; ?> href="<?php echo $settings['large']; ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
			<?php
			//setup terms
