...
if ( ! function_exists( 'woo_exclude_categories_blogtemplate' ) ) {
...
if ( ! function_exists( 'woo_homepage_blog_panel_filter' ) ) {
...
if ( ! function_exists( 'woo_pagination_html5_markup' ) ) {
	...
	function woo_get_post_images( $offset = 1, $size = 'large' ) {
		...
		if ( !empty($attachments) ) {
			...
			if ( has_post_thumbnail( $id ) ) {
				...
				$output[] = array( 'url' => $large_image_url[0], 'id' => $post_thumbnail_id, 'alt' => get_post_meta( $post_thumbnail_id, '_wp_attachment_image_alt', true ) );
			} // End If Statement
			$output = array();
			$count = 0;
...
if ( ! function_exists( 'woo_portfolio_add_post_classes' ) ) {
...
if ( ! function_exists( 'woo_add_features' ) ) {
...
if ( ! function_exists( 'woo_maps_contact_output' ) ) {
					...
					<?php if($mode == 'directions'){ ?>
	 				...
	 				directions = new GDirections(map, directionsPanel);
				  	...
				  	<?php
		  				...
		  				var root = "<?php echo esc_js( get_template_directory_uri() ); ?>";
		  				var callout = <?php echo json_encode( preg_replace("/[\n\r]/","<br/>",get_option('woo_maps_callout_text')) ); ?>;
		  				var the_link = '<?php echo esc_js( get_permalink(get_the_id()) ); ?>';
		  				<?php $title = str_replace(array('&#8220;','&#8221;'),'"', $marker_title); ?>
		  				<?php $title = str_replace('&#8211;','-',$title); ?>
		  				<?php $title = str_replace('&#8217;',"`",$title); ?>
		  				<?php $title = str_replace('&#038;','&',$title); ?>
		  				var the_title = '<?php echo esc_js( html_entity_decode($title) ); ?>';
...
if ( ! function_exists( 'woo_portfolio_gallery_filter' ) ) {
	function woo_portfolio_gallery_filter( $q ) {
		global $woo_options;
		if ( is_tax( 'portfolio-gallery' ) && $woo_options['woo_portfolio_enable_pagination'] == 'true' ) {
...
/*-----------------------------------------------------------------------------------*/
/* WPML Language ID function */
/*-----------------------------------------------------------------------------------*/

/**
 * woo_lang_id WPML compatibility function to get translated post ID
 * @param  integer $id           original post ID
 * @param  string  $content_type post type/taxonomy to translate
 * @param  boolean $original_id  true will return the original post id if no translation found
 * @return int 	   $id           translated post ID
 */
 if ( ! function_exists( 'woo_lang_id' ) ) {
	function woo_lang_id( $id = 0, $content_type = 'post', $original_id = true ){
		if ( function_exists( 'icl_object_id' ) ) {
			return icl_object_id( $id, $content_type, $original_id );
		} else {
			return $id;
		} // End If Statement
	} // End woo_lang_id()
